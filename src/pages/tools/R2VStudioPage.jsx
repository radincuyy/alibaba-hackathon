...
export default function R2VStudioPage() {
    const [references, setReferences] = useState([]); // { preview (base64), type, id }
    ...
    /**
     * Ensure image meets minimum size for API (min 384px)
     */
    ...
    const handleGenerate = async () => {
        ...
        try {
            // Send base64 data URIs directly as reference_urls
    ...
    const currentSizeOptions = resolutionTiers[resolution];

    const handleResolutionChange = (newRes) => {
        setResolution(newRes);
        // Find equivalent aspect ratio in new tier
        const currentOption = resolutionTiers[resolution].find((s) => s.value === size);
        const currentLabel = currentOption?.label || '16:9';
        const newOption = resolutionTiers[newRes].find((s) => s.label === currentLabel);
        setSize(newOption ? newOption.value : resolutionTiers[newRes][0].value);
    };
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center shadow-lg">
                            <Film className="w-6 h-6 text-cream-900" />
                        </div>
                        <div>
                    ...
                    <p className="text-cream-500 mb-8 max-w-2xl">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-3 gap-3 mb-4">
                                    {references.map((ref, idx) => (
                                        <div key={ref.id} className="relative group rounded-xl overflow-hidden border border-cream-300 bg-cream-50">
                                            ...
                                            >
                                                <Trash2 className="w-3 h-3 text-cream-900" />
                                            </button>
                                        </div>
                                    ))}
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                        {promptSuggestions.map((s, i) => (
                                            <button
                                                key={i}
                                                onClick={() => setPrompt(s)}
                                                className="text-[12px] px-2 py-1 rounded-lg bg-cream-100 border border-cream-500/20 text-cream-600 hover:text-cream-900 hover:bg-violet-500/20 transition-colors truncate max-w-[320px]"
                                            >
                                                {s.substring(0, 55)}...
                                            </button>
                                        ))}
                                    ...
                                    <div className="flex gap-2">
                                        {[2, 5, 10].map((d) => (
                                            <button
                                                ...
                                                className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${duration === d
                                    ...
                                    <div className="flex gap-2">
                                        {['720P', '1080P'].map((res) => (
                                            <button
                                                ...
                                                className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${resolution === res
                                                    ? 'bg-violet-500/20 border border-violet-500/50 text-cream-900'
                                                    : 'bg-cream-100 border border-cream-300 text-cream-400 hover:text-cream-600'
                                                    }`}
                                            >
                                                {res}
                                                <span className="text-[9px] opacity-50 ml-1">{res === '1080P' ? '(HD)' : ''}</span>
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                    ...
                                    <div className="grid grid-cols-5 gap-1.5">
                                        {currentSizeOptions.map((s) => (
                                            <button
                                                ...
                                                className={`py-2 px-1 rounded-xl text-xs font-medium transition-all text-center ${size === s.value
                                                    ? 'bg-violet-500/20 border border-violet-500/50 text-cream-900'
                                                    : 'bg-cream-100 border border-cream-300 text-cream-400 hover:text-cream-600'
                                                    }`}
                                            >
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                ...
                                                className={`flex-1 py-2.5 rounded-xl text-xs font-medium transition-all ${shotType === 'single'
                                                    ? 'bg-violet-500/20 border border-violet-500/50 text-cream-900'
                                                    : 'bg-cream-100 border border-cream-300 text-cream-400 hover:text-cream-600'
                                                    }`}
                                            ...
                                            <button
                                                ...
                                                className={`flex-1 py-2.5 rounded-xl text-xs font-medium transition-all ${shotType === 'multi'
                                                    ? 'bg-violet-500/20 border border-violet-500/50 text-cream-900'
                                                    : 'bg-cream-100 border border-cream-300 text-cream-400 hover:text-cream-600'
                                                    }`}
                                    ...
                                    <div>
                                        ...
                                        <button
                                            ...
                                            className={`w-full py-2.5 rounded-xl text-xs font-medium transition-all flex items-center justify-center gap-1.5 ${audioEnabled
                                                ? 'bg-violet-500/20 border border-violet-500/50 text-cream-900'
                            ...
                            <button
                                ...
                                className="btn-primary w-full !py-4 !bg-gradient-to-r !from-violet-600 !to-fuchsia-600 hover:!from-violet-500 hover:!to-fuchsia-500 disabled:opacity-50"
                            >
                                {isGenerating ? (
                                    <>
                        ...
                        <div className="space-y-6">
                                ...
                                {isGenerating ? (
                                    <div className="flex-1 flex flex-col items-center justify-center text-cream-400">
                                        <div className="relative">
                                            ...
                                            <Film className="w-8 h-8 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-violet-400" />
                                        </div>
                            ...
                            {/* Example */}
                            <div className="card p-5 bg-cream-50 border-cream-300">
